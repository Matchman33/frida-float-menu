...
function setDialogOverlayType(dialog) {
    try {
...
class BaseInputButton extends UIComponent {
    ...
    createBaseView(context) {
        ...
        const iconLp = LinearLayoutParams.$new(ViewGroupLayoutParams.WRAP_CONTENT.value, ViewGroupLayoutParams.WRAP_CONTENT.value);
...
export class NumberInput extends BaseInputButton {
    constructor(id, initialValue = 0, title = "请输入数值", hint = "请输入数值") {
        super(id, title, hint);
        this.min = null;
        this.max = null;
    ...
    createView(context) {
        ...
        const clickListener = Java.registerClass({
            name: "com.frida.NumberInputButtonClick" +
                Date.now() +
    ...
    showDialog(context) {
        Java.scheduleOnMainThread(() => {
            try {
                ...
                container.setPadding(dp(context, 16), dp(context, 12), dp(context, 16), dp(context, 8));
                container.addView(input);
                builder.setView(container);
                const self = this;
                builder.setPositiveButton(JString.$new("确认"), Java.registerClass({
                    ...
                    methods: {
                        onClick: function (_dialog, _which) {
                            ...
                            try {
                                const raw = Java.cast(input.getText(), Java.use("java.lang.CharSequence")).toString() + "";
                ...
                builder.setNegativeButton(JString.$new("取消"), Java.registerClass({
                    name: "com.frida.NumberInputCancel" +
                        Date.now() +
                        Math.random().toString(36).substring(6),
                    implements: [API.DialogInterfaceOnClickListener],
                    methods: {
                ...
                this.isShowDialog = false;
                Logger.instance.error(`[NumberInput:${this.id}] Failed to show dialog: ${e}`);
            }
        });
    }
...
export class TextInput extends BaseInputButton {
    constructor(id, initialValue = "", title = "请输入文本", hint = "请输入文本") {
        super(id, title, hint);
        this.value = initialValue;
    }
    buildDisplayText() {
    ...
    createView(context) {
        ...
        const self = this;
        const clickListener = Java.registerClass({
            name: "com.frida.TextInputButtonClick" +
                Date.now() +
                Math.random().toString(36).substring(6),
            implements: [API.OnClickListener],
            methods: {
        ...
        Java.scheduleOnMainThread(() => {
            try {
                ...
                builder.setNegativeButton(String.$new("取消"), Java.registerClass({
                    name: "com.frida.TextInputCancel" +
                        Date.now() +
                        Math.random().toString(36).substring(6),
                    implements: [API.DialogInterfaceOnClickListener],
                    methods: {
                ...
                this.isShowDialog = false;
                Logger.instance.error(`[TextInput:${this.id}] Failed to show dialog: ${e}`);
            }
        });
    }
    setText(text) {
