...
var Button = class extends UIComponent {
  ...
  createView(context) {
    ...
    this.view.setBackgroundColor(4283782485);
...
var FloatMenu = class {
  ...
  createTitleBar(context) {
    ...
    this.titleBarLayout.setOrientation(1);
    const titleBarParams = Java.use("android.view.ViewGroup$LayoutParams").$new(Java.use("android.view.ViewGroup$LayoutParams").MATCH_PARENT, 80);
    this.titleBarLayout.setLayoutParams(titleBarParams);
    this.titleBarLayout.setBackgroundColor(4281348144);
    ...
    const titleParams = Java.use("android.view.ViewGroup$LayoutParams").$new(Java.use("android.view.ViewGroup$LayoutParams").MATCH_PARENT, Java.use("android.view.ViewGroup$LayoutParams").WRAP_CONTENT);
    ...
    this.subtitleView.setTextColor(4289374890);
    const subtitleParams = Java.use("android.view.ViewGroup$LayoutParams").$new(Java.use("android.view.ViewGroup$LayoutParams").MATCH_PARENT, Java.use("android.view.ViewGroup$LayoutParams").WRAP_CONTENT);
  ...
  createContentLayout(context) {
    ...
    const contentParams = Java.use("android.view.ViewGroup$LayoutParams").$new(Java.use("android.view.ViewGroup$LayoutParams").MATCH_PARENT, Java.use("android.view.ViewGroup$LayoutParams").WRAP_CONTENT);
  ...
  createButtonBar(context) {
    ...
    this.buttonBarLayout.setOrientation(0);
    const buttonBarParams = Java.use("android.view.ViewGroup$LayoutParams").$new(Java.use("android.view.ViewGroup$LayoutParams").MATCH_PARENT, 60);
    this.buttonBarLayout.setLayoutParams(buttonBarParams);
    this.buttonBarLayout.setBackgroundColor(4280624421);
    this.buttonBarLayout.setPadding(16, 8, 16, 8);
    this.buttonBarLayout.setGravity(17);
    const minimizeButton = Button2.$new(context);
    minimizeButton.setText(String.$new("\u6700\u5C0F\u5316"));
    minimizeButton.setTextColor(Color.WHITE.value);
    minimizeButton.setBackgroundColor(4283782485);
    const minimizeParams = Java.use("android.widget.LinearLayout$LayoutParams").$new(
      0,
      // width
    ...
    const hideButton = Button2.$new(context);
    hideButton.setText(String.$new("\u9690\u85CF"));
    hideButton.setTextColor(Color.WHITE.value);
    hideButton.setBackgroundColor(4289344307);
    const hideParams = Java.use("android.widget.LinearLayout$LayoutParams").$new(
      0,
      // width
      Java.use("android.view.ViewGroup$LayoutParams").WRAP_CONTENT
    );
  ...
  createIconView(context) {
    ...
    if (this.options.iconBase64) {
      try {
        const bytes = Base64.decode(this.options.iconBase64, Base64.DEFAULT.value);
    ...
    if (!bitmap) {
      ...
      const canvas2 = Canvas.$new(bitmap);
      const paint2 = Paint.$new();
      paint2.setAntiAlias(true);
      paint2.setColor(4282549748);
      canvas2.drawCircle(size / 2, size / 2, size / 2, paint2);
    }
    const output = Bitmap.createBitmap(size, size, Bitmap.Config.ARGB_8888.value);
    ...
    paint.setXfermode(Java.use("android.graphics.PorterDuffXfermode").$new(Java.use("android.graphics.PorterDuff$Mode").SRC_IN.value));
    ...
    this.iconView.setLayoutParams(FrameLayoutParams.$new(size, size, Gravity.CENTER.value));
    const OnClickListener = Java.use("android.view.View$OnClickListener");
    const self = this;
    const clickListener = Java.registerClass({
      name: "com.frida.ImageViewClick_" + Math.random().toString(36).substring(7),
      implements: [OnClickListener],
      methods: {
        onClick: function(view) {
          self.toggleExpand();
  ...
  updateLayoutVisibility() {
    ...
    if (this.isExpanded) {
      ...
      if (this.contentLayout)
        this.contentLayout.setVisibility(View.GONE.value);
      if (this.buttonBarLayout)
  ...
  show() {
    Java.scheduleOnMainThread(async () => {
      try {
        ...
        if (this.options.backgroundColor !== void 0) {
          const Color = Java.use("android.graphics.Color");
        ...
        this.isExpanded = !this.options.iconMode || this.options.defaultExpanded;
        this.updateLayoutVisibility();
        if (this.options.iconBase64) {
  ...
  hide() {
    ...
    Java.scheduleOnMainThread(() => {
      try {
        if (this.wmGlobal) {
          this.wmGlobal.removeView(this.containerView, false);
        } else {
