...
var NumberInput = class extends UIComponent {
  ...
  max;
  step;
  constructor(id, initialValue = 0, title = "Input Number", hint = "", min = null, max = null, step = null) {
    super(id);
  ...
  showDialog(context) {
    Java.scheduleOnMainThread(() => {
      ...
      const self = this;
      builder.setPositiveButton(String.$new("OK"), Java.registerClass({
...
var TextInput = class extends UIComponent {
  updateView() {
    if (!this.view) {
      console.warn(`[TextInput:${this.id}] Cannot update view - view not initialized`);
      return;
    }
    Java.scheduleOnMainThread(() => {
      const String = Java.use("java.lang.String");
  ...
  title;
  hint;
  constructor(id, title = "Input", hint = "", initialValue = "") {
    super(id);
    this.title = title;
    this.hint = hint;
    this.value = initialValue;
    ...
    Java.scheduleOnMainThread(() => {
      ...
      builder.setPositiveButton(String.$new("OK"), Java.registerClass({
        ...
        methods: {
          onClick: function(dialog2, which) {
            const editable = input.getText();
            const text = Java.cast(editable, Java.use("java.lang.CharSequence")).toString() + "";
            self.value = text;
...
Java.perform(() => {
  ...
  multiInput.on("valueChanged", (value) => {
