...
var FloatMenu = class {
  ...
  updateWindowPosition() {
    const { x: iconWX, y: iconWY } = this.logicalToWindow(this.logicalX, this.logicalY);
    this.iconWindowParams.x.value = iconWX | 0;
    this.iconWindowParams.y.value = iconWY | 0;
    const menuOffsetX = 0;
    const menuOffsetY = this.options.iconHeight ?? 0;
    const { x: menuWX, y: menuWY } = this.logicalToWindow(this.logicalX + menuOffsetX, this.logicalY + menuOffsetY);
            ...
            switch (action) {
              ...
              case MotionEvent.ACTION_MOVE.value: {
                ...
                if (Math.abs(dx) > DRAG_THRESHOLD || Math.abs(dy) > DRAG_THRESHOLD) {
                  ...
                  let newX = self.initialWindowX + dx;
                  let newY = self.initialWindowY + dy;
                  const { x: x2, y: y2 } = self.windowToLogical(newX, newY);
                  newX = x2;
                  newY = y2;
                  newX = Math.max(bounds.left, Math.min(bounds.right, newX));
                  newY = Math.max(bounds.top, Math.min(bounds.bottom, newY));
      ...
      const clickListener = Java.registerClass({
        ...
        methods: {
          onClick: function() {
            ...
            self.windowManager.updateViewLayout(self.menuContainerView, self.menuWindowParams);
            self.toggleView();
          }
        }
      });
