...
var FloatMenu = class {
  ...
  enableDragForView(targetView, options) {
          ...
          switch (action) {
            ...
            case MotionEvent.ACTION_MOVE.value:
              ...
              if (Math.abs(dx) > DRAG_THRESHOLD || Math.abs(dy) > DRAG_THRESHOLD) {
                ...
                self.windowManager.updateViewLayout(self.iconContainerView, self.iconWindowParams);
              }
              return isDragging;
            case MotionEvent.ACTION_UP.value:
              if (isDragging && snapToEdge) {
                ...
                if (self.iconWindowParams.x < midX) {
                  self.iconWindowParams.x = 0;
                } else {
                  self.iconWindowParams.x = self.screenWidth.value - self.options.iconWidth;
                }
                self.windowManager.updateViewLayout(self.iconContainerView, self.iconWindowParams);
              }
              return isDragging;
  ...
  createIconWindow() {
    try {
      ...
      this.iconWindowParams = LayoutParams.$new(this.options.iconWidth, this.options.iconHeight, this.options.x, this.options.y, 2038, LayoutParams.FLAG_NOT_FOCUSABLE.value | LayoutParams.FLAG_NOT_TOUCH_MODAL.value, 1);
      const FrameLayout = API.FrameLayout;
