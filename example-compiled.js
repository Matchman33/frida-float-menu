...
var FloatMenu = class {
  ...
  createIconView() {
    try {
      ...
      const self = this;
      const clickListener = Java.registerClass({
        name: "com.frida.ClickListener" + Date.now() + Math.random().toString(36).substring(6),
        implements: [OnClickListener],
        methods: {
  ...
  enableDragForView(targetView) {
    ...
    const touchListener = Java.registerClass({
      name: "com.example.FloatDragListener" + Date.now() + Math.random().toString(36).substring(6),
          ...
          switch (action) {
            ...
            case MotionEvent.ACTION_MOVE.value:
              ...
              if (Math.abs(dx) > DRAG_THRESHOLD || Math.abs(dy) > DRAG_THRESHOLD) {
                ...
                self.windowManager.updateViewLayout(self.parentContainerView, self.windowParams);
              }
              return isDragging;
...
Java.perform(() => {
  ...
  genderGroup.on("change", (checkedValues, lastChange) => {
    ...
    if (lastChange) {
      console.log(`\u9009\u9879 ${lastChange.id} \u53D8\u4E3A ${lastChange.checked ? "\u9009\u4E2D" : "\u672A\u9009\u4E2D"}`);
    }
  });
